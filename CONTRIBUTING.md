...
Commit the engine-produced `run_summary.json` + `provenance.json` under `results/omnidocbench/v1.6/<platform>/` and update `model_card.json.artifacts` to point at them. A maintainer will review for the `verified` badge per the policy in `omnidocbench-rocm/contracts/badge-policy.md`.
