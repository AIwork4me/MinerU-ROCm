...
from mineru_rocm.backends import pipeline, vlm
...
def test_smoke_backend_writes_md_and_stats(tmp_path):
    _write_img(tmp_path)
    out = tmp_path / "out"
    dispatcher.run_adapter(tmp_path, out, platform="linux-rocm", config={"backend": "smoke"})
...
def test_pipeline_backend_routes_to_pipeline_adapter(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pipeline, "infer_page", fake)
    out = tmp_path / "out"
...
def test_vlm_backend_routes_to_vlm_adapter(tmp_path, monkeypatch):
    _write_img(tmp_path)
    monkeypatch.setattr(vlm, "infer_page", lambda i, p, c: "# vlm md\n")
    out = tmp_path / "out"
    dispatcher.run_adapter(tmp_path, out, platform="linux-rocm", config={"backend": "vlm-transformers"})
    assert (out / "p1.md").read_text(encoding="utf-8") == "# vlm md\n"

def test_unknown_backend_raises_value_error(tmp_path):
    _write_img(tmp_path)
    try:
        dispatcher.run_adapter(tmp_path, tmp_path / "o", platform="linux-rocm", config={"backend": "wat"})
